...
static id DBValueForKeyPath(id obj, NSString *keyPath) {
    NSArray *keys = [keyPath componentsSeparatedByString:@"."];
    
    id value = obj;
    for (NSString *key in keys) {
        if ([value isKindOfClass:[NSDictionary class]] || [value isKindOfClass:[DBObject class]]) {
    ...
    [mapper enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
        @try {
            ...
            if ([obj isKindOfClass:[NSString class]] && [obj length]) {
                value = DBValueForKeyPath(object, obj);
            }
                ...
                for (NSString *oneKey in ((NSArray *)obj)) {
                    ...
                    value = DBValueForKeyPath(object, oneKey);
                }
            }
